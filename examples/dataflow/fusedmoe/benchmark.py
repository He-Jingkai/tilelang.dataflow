...
if TYPE_CHECKING:
    from tilelang.dataflow import DataflowExecutionPlan, TargetCapabilitySnapshot
...
# Performance references only: these must never participate in scheduling.
# Use the same-environment original-path controls for unchanged workloads.
# DeepSeek-128 retains the prior winning three-process median, not a slower
# new control. These references were measured on 2026-09-09.
NONREGRESSION_H100_P50_US = {
...
AUTOMATIC_PER_POINT_GATE_PERCENT = 0.5
AUTOMATIC_PER_POINT_GATE_MIN_US = 1.0


def h100_latency_guard(workload: str, p50_us: float, *, automatic: bool) -> dict[str, float | bool]:
    """Use same-environment controls or August references; reject bad timings.

    The pinned optimized profile shares the current automatic reference so
    both entry points protect the same low-latency execution contract.
    """
    if not math.isfinite(p50_us) or p50_us <= 0:
        raise ValueError(f"p50_us must be finite and positive, got {p50_us}")
    references = NONREGRESSION_H100_P50_US if automatic else PUBLISHED_H100_P50_US
    reference = references[workload]
    gate = AUTOMATIC_PER_POINT_GATE_PERCENT if automatic else PUBLISHED_PER_POINT_GATE_PERCENT
    allowed_delta_us = max(reference * gate / 100.0, AUTOMATIC_PER_POINT_GATE_MIN_US if automatic else 0.0)
    gate = allowed_delta_us / reference * 100.0
    regression = (p50_us / reference - 1.0) * 100.0
    return {
        "reference_p50_us": reference,
        "regression_percent": regression,
        "gate_percent": gate,
        "allowed_delta_us": allowed_delta_us,
        "passes": regression <= gate + 1.0e-9,
        "historical_regression_percent": (p50_us / PUBLISHED_H100_P50_US[workload] - 1.0) * 100.0,
    }


# Explicit, measured execution profiles. Keep the published profiles available
# as controls: the same operator and cluster-shared transport execute both.
OPTIMIZED_H100_WORKLOADS: dict[str, dict[str, Any]] = {
    "deepseek-128": {
        **PUBLISHED_H100_WORKLOADS["deepseek-128"],
        # SM90 MegaMoE tunes each GEMM's K/N tiles and pipeline together.
        # A wider first reduction tile requires narrower expert tiles here;
        # the linked second GEMM then benefits from four smaller weight stages.
        "block_dhidden": 256,
        "block_dexpert": 128,
        "hot_cluster_size": 4,
        "cold_cluster_size": 4,
        "hot_map1_handler_dexpert": 512,
        "cold_block_dhidden": 256,
        "cold_block_dexpert": 128,
        "map2_weight_stages": 4,
        "map2_weight_max_outstanding": 4,
        "cold_map2_weight_stages": 4,
        "cold_map2_weight_max_outstanding": 4,
    },
}
...
def select_dataflow_small_batch_execution(
    counts: Iterable[int],
    *,
    d_hidden: int,
    d_expert: int,
...
) -> tuple[DataflowExecutionPlan, DataflowExecutionPlan] | None:
    """Joint ragged-task placement using the common execution planner.

    This optional candidate class is for small, long-reduction batches. It
    contains no workload names, token-count presets or measured latency table.
    Wider batches retain the existing wave-balanced/streamed scheduling path.
    """
    import tilelang.dataflow as df

    counts = tuple(counts)
    if not counts or max(counts) > 32 or sum(count > 0 for count in counts) < 2 or d_hidden < 16 * 256:
        return None
    if not (target_capabilities.supports_tma and target_capabilities.supports_wgmma and target_capabilities.supports_cluster_launch):
        return None

    def blocks(tile):
        result = []
        for expert, count in enumerate(counts):
            for begin in range(0, count, tile):
                result.append(RoutedGroupBlock(len(result), expert, min(tile, count - begin)))
        return tuple(result)

    hot_blocks, cold_blocks = blocks(32), blocks(16)
    candidates = []
    # A common cluster class keeps both buckets' per-CTA weight work balanced;
    # only the number of clusters and the token tile differ between buckets.
    for cluster in range(1, (target_capabilities.max_cluster_size or 16) + 1):
        if any(extent % cluster for extent in (hot_sm_count, cold_sm_count, d_hidden, d_expert)):
            continue
        if cold_expert_count is not None and cold_expert_count > cold_sm_count // cluster:
            continue
        schedule = build_dual_kernel_schedule(
            hot_blocks,
            cold_blocks=cold_blocks,
            hot_cluster_count=hot_sm_count // cluster,
            cold_cluster_count=cold_sm_count // cluster,
            cold_expert_count=cold_expert_count,
        )
        plans = []
        for bucket, sm_count, tile, outstanding in (
            (schedule.split.hot_blocks, hot_sm_count, 32, 2),
            (schedule.split.cold_blocks, cold_sm_count, 16, 3),
        ):
            extents = tuple(block.actual_rows for block in bucket)
            request = df.DataflowExecutionRequest(
                task_extent=sum(extents),
                task_group_extents=extents,
                stages=(
                    df.DataflowExecutionStageRequest(
                        d_expert, d_hidden, "float8_e4m3fn", "float8_e4m3fn", "float8_e4m3fn", projection_count=2
                    ),
                    df.DataflowExecutionStageRequest(
                        d_hidden, d_expert, "float8_e4m3fn", "float8_e4m3fn", "float16", input_from_previous_stage=True
                    ),
                ),
            ...
            try:
                plan = df.plan_execution(
                    ...
                    override=df.DataflowExecutionOverride(
                        topology=df.GPUTopology(sm_count, cluster),
                        task_tile_extent=tile,
                        stages=(
                            df.DataflowExecutionStageOverride(pipeline_stages=3, max_outstanding=outstanding),
            ...
            except df.DataflowExecutionPlanningError:
                break
            first, second = plan.selected_candidate.stages
            if (first.tile_k, first.tile_n, second.tile_n, second.pipeline_stages) != (256, 128, 256, 4):
                break
            plans.append(plan)
        if len(plans) == 2:
            costs = tuple(plan.selected_evaluation.score[1] for plan in plans)
            candidates.append(((max(costs), sum(costs), -cluster), tuple(plans)))
    return min(candidates, key=lambda item: item[0])[1] if candidates else None


def automatic_dataflow_token_tile(counts: Iterable[int], *, d_hidden: int) -> int:
    """Coarsen long reductions; retain more row parallelism for short ones."""
    blocks_per_expert = 2 if d_hidden >= 16 * 256 else 4
    rows = math.ceil(max(counts) / blocks_per_expert)
    return min(128, max(32, 1 << max(0, rows - 1).bit_length()))
...
def profile_dataflow_dual_routed_moe(
    ...
    *,
    ...
    """Preserve direct callers' geometry; the CLI explicitly opts into auto."""
    import tilelang.language as T
    import tilelang.dataflow as df

    from examples.dataflow.fusedmoe import example_fusedmoe_dataflow as dataflow_moe

    if execution_policy not in {"auto", "manual"}:
        raise ValueError(f"unsupported execution_policy {execution_policy!r}")
    counts = scale_expert_heat(EXPERT_HEAT, total_assignments=token_num * top_k)
    automatic_plans = None
    if execution_policy == "auto" and use_fp8 and use_tma_weights and map1_input_mode == "unicast":
        target = df.resolve_target_capabilities()
        block_token = automatic_dataflow_token_tile(counts, d_hidden=d_hidden)
        # Keep the intermediate on-chip: materialize it when it fits, otherwise
        # retain the existing streamed producer-push/credit protocol.
        streamed = target.max_dynamic_shared_memory is not None and block_token * d_expert >= target.max_dynamic_shared_memory
        reshared_policy = "cluster_shared_pull_ring" if streamed else "cluster_shared_all_gather"
        cold_reshared_policy = "cluster_shared_all_gather"
        fuse_map2_hidden_tiles = d_hidden >= 16 * 256 and block_token >= 64
        if streamed:
            map2_weight_stages = 2
            map2_map1_lookahead_stages = 0
            cold_map1_gate_stages = 2
        automatic_plans = select_dataflow_small_batch_execution(
            counts,
            d_hidden=d_hidden,
            d_expert=d_expert,
        ...
        if automatic_plans is not None:
            hot, cold = (plan.selected_candidate for plan in automatic_plans)
            block_token, cold_block_token = hot.task_tile_extent, cold.task_tile_extent
            hot_cluster_size, cold_cluster_size = hot.topology.cluster_size, cold.topology.cluster_size
            block_dhidden, block_dexpert = hot.stages[0].tile_k, hot.stages[0].tile_n
            cold_block_dhidden, cold_block_dexpert = cold.stages[0].tile_k, cold.stages[0].tile_n
            hot_map1_handler_dexpert = hot.stages[0].handler_extent
            cold_fuse_map1_expert_shards = True
            map2_block_dhidden, cold_map2_block_dhidden = hot.stages[1].tile_n, cold.stages[1].tile_n
            hot_map2_handler_dhidden, cold_map2_handler_dhidden = hot.stages[1].handler_extent, cold.stages[1].handler_extent
            map2_weight_stages, map2_weight_max_outstanding = hot.stages[1].pipeline_stages, hot.stages[1].max_outstanding
            cold_map2_weight_stages, cold_map2_weight_max_outstanding = cold.stages[1].pipeline_stages, cold.stages[1].max_outstanding
            cold_map1_gate_stages = cold.stages[0].pipeline_stages
            map2_map1_lookahead_stages = 0
            fuse_map2_hidden_tiles = False
            print(
                f"AUTO_EXECUTION phase_specific_small_m hot_cluster={hot_cluster_size} cold_cluster={cold_cluster_size} "
                f"map1_k={block_dhidden} map1_n={block_dexpert} map2_stages={map2_weight_stages}",
                flush=True,
            )
        ...
        *,
        ...
        try:
            return dataflow_moe.compile_dataflow_routed_moe(
                ...
                task_group_extents=tuple(block.actual_rows for block in blocks),
                stage_graph_task_weights=task_costs,
                stage_graph_cluster_assignment=cluster_assignment,
            )
    ...
    if expected_output is not None:
        ...
        relative_l2_limit = 0.02 if use_fp8 else 0.002
        if not math.isfinite(verification_relative_l2) or verification_relative_l2 > relative_l2_limit:
            raise RuntimeError(f"routed-MoE reference check failed: relative_l2={verification_relative_l2}, limit={relative_l2_limit}")
        if not torch.equal(concurrent_output.view(torch.int32), sequential_output.view(torch.int32)):
            raise RuntimeError("routed-MoE concurrent and sequential outputs are not bitwise identical")
        if instrumented_concurrent_output is not None and not torch.equal(
            instrumented_concurrent_output.view(torch.int32), concurrent_output.view(torch.int32)
        ):
            raise RuntimeError("routed-MoE instrumented and production outputs are not bitwise identical")
    row = base_result(
        "dataflow-dual",
        token_num=token_num,
        top_k=top_k,
    ...
    row.update(
        {
            ...
            "execution_selection": "phase_specific_small_m" if automatic_plans is not None else "wave_balanced",
            "automatic_execution_candidates": None
            if automatic_plans is None
            else [plan.selected_candidate.to_dict() for plan in automatic_plans],
            "automatic_execution_scores": None if automatic_plans is None else [plan.selected_evaluation.score for plan in automatic_plans],
            "hot_sm_count": hot_sm_count,
            "hot_cluster_size": hot_cluster_size,
            ...
            "hot_block_dhidden": block_dhidden,
            "hot_block_dexpert": block_dexpert,
            "cold_block_token": cold_block_token,
...
def apply_published_h100_workload(args: argparse.Namespace) -> None:
    optimized_workload = getattr(args, "optimized_h100_workload", None)
    workload_name = optimized_workload or args.published_h100_workload
    if workload_name is None:
        return
    profiles = OPTIMIZED_H100_WORKLOADS if optimized_workload else PUBLISHED_H100_WORKLOADS
    profile = profiles[workload_name]
    args.dataflow_execution_policy = "manual"
    args.backend = "dataflow-dual"
    args.token_num = [profile["token_num"]]
    args.d_hidden = profile["d_hidden"]
    args.d_expert = profile["d_expert"]
    args.top_k = 2
    args.block_dhidden = profile.get("block_dhidden", 128)
    args.block_dexpert = profile.get("block_dexpert", 128)
    args.warmups = 5
    args.repeats = 30
    args.disable_cache = True
    args.dataflow_hot_sm_count = 112
    args.dataflow_hot_cluster_size = profile.get("hot_cluster_size")
    args.dataflow_cold_sm_count = 20
    args.dataflow_cold_cluster_size = profile.get("cold_cluster_size", 2)
    ...
    args.dataflow_cold_block_dhidden = profile.get("cold_block_dhidden", 128)
    args.dataflow_cold_map2_block_dhidden = 256
    args.dataflow_cold_map2_handler_dhidden = None
    args.dataflow_cold_block_dexpert = profile.get("cold_block_dexpert", 256)
    ...
    args.dataflow_hot_map1_handler_dexpert = profile.get("hot_map1_handler_dexpert")
    args.dataflow_map2_block_dhidden = 256
    args.dataflow_map2_weight_stages = profile.get("map2_weight_stages")
    args.dataflow_map2_weight_max_outstanding = profile.get("map2_weight_max_outstanding", 2)
    args.dataflow_cold_map2_weight_stages = profile.get("cold_map2_weight_stages", 2)
    args.dataflow_cold_map2_weight_max_outstanding = profile.get("cold_map2_weight_max_outstanding", 2)
...
def parse_args_from_list(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    workload_profiles = parser.add_mutually_exclusive_group()
    workload_profiles.add_argument(
        "--h100-workload",
        choices=tuple(PUBLISHED_H100_WORKLOADS),
        help="Select only the matrix shape and 5-warmup/30-sample protocol; use automatic execution planning.",
    )
    workload_profiles.add_argument(
        "--published-h100-workload",
        choices=tuple(PUBLISHED_H100_WORKLOADS),
        help="Reproduce an original 2026-08-18 workload profile and its historical 3% gate.",
    )
    workload_profiles.add_argument(
        "--optimized-h100-workload",
        choices=tuple(OPTIMIZED_H100_WORKLOADS),
        help=("Apply a measured low-latency execution profile with the same operator, FP8 precision, and on-chip all-gather."),
    )
    ...
    parser.add_argument(
        "--dataflow-execution-policy",
        choices=("auto", "manual"),
        default=None,
        help="Default: auto. Explicit tiling/pipeline/transport flags select manual execution; historical profiles are always manual.",
    )
    parser.add_argument("--result-json", help="Save the complete measured row and selected scheduling facts as JSON.")
    ...
    # Geometry controls remain authoritative, including explicit values equal
    # to old defaults. Do not silently retune a user-specified execution path.
    controls = {
        action.dest
        for action in parser._actions
        if action.dest.startswith(
            ("block_", "dataflow_hot_", "dataflow_cold_", "dataflow_map", "dataflow_block_", "dataflow_fuse_", "dataflow_reshared_")
        )
        and action.dest not in {"dataflow_hot_sm_count", "dataflow_cold_sm_count", "dataflow_cold_expert_count"}
    }
    explicit_controls = {
        action.dest
        for action in parser._actions
        if action.dest in controls and any(arg.split("=", 1)[0] in action.option_strings for arg in argv)
    }
    if explicit_controls and (args.dataflow_execution_policy == "auto" or args.h100_workload):
        parser.error("automatic execution and explicit geometry controls are mutually exclusive; use --dataflow-execution-policy manual")
    if args.dataflow_execution_policy is None:
        args.dataflow_execution_policy = "manual" if explicit_controls else "auto"
    if (args.h100_workload or args.published_h100_workload or args.optimized_h100_workload) and args.dataflow_walltime_dir is None:
        parser.error("H100 workload profiles require --dataflow-walltime-dir")
    apply_published_h100_workload(args)
    if args.h100_workload:
        if args.dataflow_execution_policy == "manual":
            parser.error("--h100-workload requires automatic execution")
        if args.no_dataflow_fp8 or args.no_dataflow_tma_weights:
            parser.error("--h100-workload requires the FP8/TMA measurement protocol")
        shape = PUBLISHED_H100_WORKLOADS[args.h100_workload]
        args.backend, args.token_num = "dataflow-dual", [shape["token_num"]]
        args.d_hidden, args.d_expert, args.top_k = shape["d_hidden"], shape["d_expert"], 2
        args.warmups, args.repeats, args.dataflow_walltime_repeats = 5, 30, 30
        args.disable_cache = True
        args.verify_output = True
    if args.result_json and (len(args.token_num) != 1 or args.backend in {"both", "all"}):
        parser.error("--result-json requires one workload/backend")
    return args
...
def main() -> None:
    args = parse_args()
    workload_name = args.h100_workload or args.optimized_h100_workload or args.published_h100_workload
        ...
        for backend in selected_backends(args.backend):
            if backend == "dataflow":
                ...
                row = profile_dataflow_dual_routed_moe(
                    ...
                    execution_policy=args.dataflow_execution_policy,
                )
            print_result(row)
            guard = None
            if workload_name is not None:
                guard = h100_latency_guard(
                    workload_name,
                    float(row["dual_walltime_p50_us"]),
                    automatic=bool(args.h100_workload or args.optimized_h100_workload),
                )
                row["performance_guard"] = {**guard, "enforced": True}
            if args.result_json:
                result_path = Path(args.result_json)
                result_path.parent.mkdir(parents=True, exist_ok=True)
                result_path.write_text(json.dumps({"workload": workload_name, "arguments": vars(args), **row}, indent=2) + "\n")
            if workload_name is not None:
                p50_us = float(row["dual_walltime_p50_us"])
                assert guard is not None
                reference_p50_us = guard["reference_p50_us"]
                regression_percent = guard["regression_percent"]
                result_label = (
                    "AUTO_RESULT" if args.h100_workload else "OPTIMIZED_RESULT" if args.optimized_h100_workload else "PUBLISHED_RESULT"
                )
                print(
                    f"{result_label} "
                    ...
                    f"gate_percent={guard['gate_percent']:.3f} gate_us={guard['allowed_delta_us']:.3f} "
                    f"passes={guard['passes']} gate_enforced=True "
                    f"historical_regression_percent={guard['historical_regression_percent']:+.3f} "
                    f"warmups={args.warmups} samples={args.dataflow_walltime_repeats}"
                )
                if not guard["passes"]:
                    raise RuntimeError(
                        f"{workload_name} regressed {regression_percent:.3f}% against the {reference_p50_us:.3f} us reference "
                        f"(allowed {guard['gate_percent']:.3f}% / {guard['allowed_delta_us']:.3f} us)"
                    )
