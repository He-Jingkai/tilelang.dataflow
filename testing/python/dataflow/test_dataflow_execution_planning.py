...
@pytest.mark.parametrize("groups", [(17, 0, 15), (1,) * 32])
def test_execution_ragged_task_facts_round_trip_and_fingerprint(groups):
    dense = make_request(task_extent=sum(groups))
    ragged = replace(dense, task_group_extents=groups)
    assert ragged.fingerprint != dense.fingerprint
    assert df.DataflowExecutionRequest.from_dict(json.loads(json.dumps(ragged.to_dict()))) == ragged
    assert "task_group_extents" not in dense.to_dict()
    assert df.DataflowExecutionRequest.from_dict(dense.to_dict()) == dense


@pytest.mark.parametrize("groups", [(31,), (-1, 33), (True, 31), (1.0, 31)])
def test_execution_ragged_task_facts_reject_invalid_extents(groups):
    with pytest.raises(ValueError):
        replace(make_request(task_extent=32), task_group_extents=groups)


def test_execution_ragged_groups_cannot_share_a_task_tile():
    override = portable_override(topology=df.GPUTopology(4, 1), task_tile=32)
    dense = make_request(task_extent=128, first_output=256, second_output=256)
    ragged = replace(dense, task_group_extents=(1,) * 128)
    dense_plan = df.plan_execution(dense, override=override, target_capabilities=make_target())
    ragged_plan = df.plan_execution(ragged, override=override, target_capabilities=make_target())
    assert dense_plan.selected_evaluation.score[1] == 1
    assert ragged_plan.selected_evaluation.score[1] == 32


@pytest.mark.parametrize("hidden,expert", [(4096, 1024), (6144, 1536), (7168, 2048), (8192, 2048)])
def test_execution_joint_phase_tiling_is_shape_driven(hidden, expert):
    request = replace(make_request(task_extent=256, first_output=expert, second_output=hidden), task_group_extents=(16,) * 16)
    plan = df.plan_execution(
        request,
        override=df.DataflowExecutionOverride(topology=df.GPUTopology(112, 4), task_tile_extent=32),
        target_capabilities=make_target(sm_count=132),
    )
    first, second = plan.selected_candidate.stages
    assert (first.tile_k, first.tile_n, second.tile_k, second.tile_n) == (256, 128, 128, 256)
    assert first.handler_extent == expert // 4
    assert second.handler_extent == min(hidden // 4, 7 * 256)
    assert (first.pipeline_stages, first.max_outstanding, second.pipeline_stages, second.max_outstanding) == (3, 2, 4, 4)
    assert plan.selected_candidate.transport_family == df.DATAFLOW_TRANSPORT_ALL_GATHER
    assert plan.resources.shared_memory_bytes <= 232_448
    assert df.DataflowExecutionPlan.from_dict(plan.to_dict()) == plan


def test_execution_joint_phase_tiling_respects_explicit_fields_and_resource_fallback():
    request = replace(make_request(task_extent=256, second_output=7168), task_group_extents=(16,) * 16)
    override = df.DataflowExecutionOverride(
        topology=df.GPUTopology(112, 4),
        task_tile_extent=32,
        stages=(
            df.DataflowExecutionStageOverride(tile_k=128, handler_extent=128),
            df.DataflowExecutionStageOverride(pipeline_stages=2, max_outstanding=2),
        ),
    )
    plan = df.plan_execution(request, override=override, target_capabilities=make_target(sm_count=132))
    first, second = plan.selected_candidate.stages
    assert (first.tile_k, first.handler_extent, second.pipeline_stages, second.max_outstanding) == (128, 128, 2, 2)
    constrained = df.plan_execution(
        request,
        override=replace(override, stages=()),
        target_capabilities=make_target(sm_count=132, shared_memory=196_608),
    )
    assert constrained.selected_evaluation.legal
    assert constrained.resources.shared_memory_bytes <= 196_608
    assert any(not item.legal and item.candidate.stages[0].tile_k == 256 for item in constrained.candidates)


@pytest.mark.parametrize("transport", [df.DATAFLOW_TRANSPORT_HBM, df.DATAFLOW_TRANSPORT_STREAMED])
def test_execution_phase_candidate_does_not_replace_explicit_transport(transport):
    request = replace(make_request(task_extent=256, second_output=7168), task_group_extents=(16,) * 16)
    plan = df.plan_execution(
        request,
        override=df.DataflowExecutionOverride(topology=df.GPUTopology(112, 4), task_tile_extent=32, transport_family=transport),
        target_capabilities=make_target(sm_count=132),
    )
    assert all(item.candidate.transport_family == transport for item in plan.candidates)
    assert plan.selected_evaluation.legal
